...
$includes = array(
				'lib/auctions-cpt.php', 
				'lib/vehicles-cpt.php', 
...
//Logos and images query
function logos_images_query( $query ) {
    if ( ! is_admin() && $query->is_category() && $query->is_main_query() ) {
        $query->set( 'posts_per_page', -1 );
	}
}
add_action( 'pre_get_posts', 'logos_images_query' );


function showroom_query( $query ) {
    if ( ! is_admin() &&  $query->is_tax( 'sales_category') && $query->is_main_query() ) {
		$query->set( 'posts_per_page', -1 );
	}
}
add_action( 'pre_get_posts', 'showroom_query' );
